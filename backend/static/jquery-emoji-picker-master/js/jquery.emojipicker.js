;(function($) {
  ...
  $.extend(EmojiPicker.prototype, {
    ...
    createPicker: function() {
      ...
      if (this.settings.width < 240) {
        this.$picker.find('.emoji').css({'width':'1rem', 'height':'1rem'});
      }
    ...
    updatePosition: function() {
      ...
      */
      ...
      if(this.settings.position == 'top') {
        elOffset.top -= this.$el.outerHeight() + this.settings.height;
      }else{
    ...
    emojiClicked: function(e) { var clickTarget = $(e.target);
      ...
      insertAtCaret(this.element, emojiUnicode, emojiShortcode);
  ...
  function getPickerHTML() {
    ...
    if (localStorageSupport) {
      nodes.push('<div class="tab active" data-tab="recent"><i class="fa fa-clock"></i></div>');
    }
  ...
  function insertAtCaret(inputField, myValue, shortcode) {
    let to_be_inserted = '<div style="width: 1.5rem; height: 1.5rem;display: inline-block; color: transparent" contenteditable="false" class="emoji-' + shortcode + '">'+ myValue + '</div>&nbsp'
    ...
    if (document.selection) {
      ...
      for (let i=0; i<inputField.childNodes.length; i++){
        let cur = inputField.childNodes[i];
        if (cur.nodeType == 3){
          careq += cur.nodeValue.length;
          if (careq >= caret_position_mes){
            offset += caret_position_mes - careq + cur.nodeValue.length;
            break;
          } else {
            offset += cur.nodeValue.length;
          }
        } else{
          offset += cur.outerHTML.length;
        }
      }

      var scrollTop = inputField.scrollTop;
      inputField.innerHTML = inputField.innerHTML.substring(0, offset)+to_be_inserted+inputField.innerHTML.substring(offset, inputField.innerHTML.length+offset);
      inputField.focus();
      inputField.scrollTop = scrollTop;
    } /*else {
      inputField.focus();
      inputField.innerHTML += to_be_inserted;
    }*/
  }
